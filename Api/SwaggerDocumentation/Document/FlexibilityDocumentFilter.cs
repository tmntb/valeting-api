...
public class FlexibilityDocumentFilter : IDocumentFilter
{
    ...
    public void Apply(OpenApiDocument swaggerDoc, DocumentFilterContext context)
    {
        ...
        flexibilitiesPaths.Operations.FirstOrDefault(x => x.Key == HttpMethod.Get).Value.OperationId = "get-flexibilities";
        flexibilitiesPaths.Operations.FirstOrDefault(x => x.Key == HttpMethod.Get).Value.Summary = "List flexibilities";
        flexibilitiesPaths.Operations.FirstOrDefault(x => x.Key == HttpMethod.Get).Value.Description = "Return a list of all **Flexibilities**, it can be filter by the page number, page size and/or active";

        var flexibilitiesIdPaths = swaggerDoc.Paths.FirstOrDefault(x => x.Key == FlexibilitiesIdEndpoint).Value;
        flexibilitiesIdPaths.Operations.FirstOrDefault(x => x.Key == HttpMethod.Get).Value.OperationId = "get-flexibilities-id";
        flexibilitiesIdPaths.Operations.FirstOrDefault(x => x.Key == HttpMethod.Get).Value.Summary = "List flexibilities by id";
        flexibilitiesIdPaths.Operations.FirstOrDefault(x => x.Key == HttpMethod.Get).Value.Description = "Returns a **Flexibility** by the given id";
    }
}